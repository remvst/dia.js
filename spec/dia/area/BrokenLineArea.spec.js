describe('a broken line area', function(){
	...
	it('can calculate positions at specific ratios', function(){
		...
		expect(round(area.getPositionAtRatio(0))).toEqual({ x: 0, y: 0, angle: -Math.PI / 4 });
		expect(round(area.getPositionAtRatio(.5))).toEqual({ x: 15, y: -5, angle: Math.PI / 4 });
		expect(round(area.getPositionAtRatio(1))).toEqual({ x: 30, y: -10, angle: -Math.PI / 4 });

		expect(round(area.getPositionAtRatio(1 / 3))).toEqual({ x: 10, y: -10, angle: -Math.PI / 4 });
		expect(round(area.getPositionAtRatio(2 / 3))).toEqual({ x: 20, y: 0, angle: Math.PI / 4 });

		expect(round(area.getPositionAtRatio(1 / 6))).toEqual({ x: 5, y: -5, angle: -Math.PI / 4 });
		expect(round(area.getPositionAtRatio(5 / 6))).toEqual({ x: 25, y: -5, angle: -Math.PI / 4 });
	});
